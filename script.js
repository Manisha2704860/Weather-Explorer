async function getWeather() {
    ...
    try {
        ...
        // Build a readable update and place it in the output box
        const condition = currentData.weather[0].main || currentData.weather[0].description;
        const humidity = currentData.main.humidity;
        const wind = currentData.wind && currentData.wind.speed ? `${currentData.wind.speed} m/s` : 'N/A';
        const updatedAt = new Date(currentData.dt * 1000).toLocaleString();
        const updateText = `As of ${updatedAt} — ${currentData.name}: ${currentData.weather[0].description}, ${currentData.main.temp}°C, Humidity ${humidity}%, Wind ${wind}.`;
        const outputBox = document.getElementById('outputBox');
        if (outputBox) outputBox.textContent = updateText;

        // Adjust background to match condition
        if (condition) changeBackground(condition.toLowerCase());
        ...
        const outputBox = document.getElementById('outputBox');
        if (outputBox) outputBox.textContent = 'Could not fetch weather data. Please check the city name or your network.';
    }
}
